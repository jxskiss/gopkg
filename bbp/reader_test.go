...
func TestReader(t *testing.T) {
	...
	tests := []struct {
		...
		{seek: io.SeekStart, off: -1, seekerr: "bbp.Reader.Seek: negative position"},
...
func TestReaderAt(t *testing.T) {
	...
	tests := []struct {
		...
		{11, 10, "", io.EOF},
		{0, 0, "", nil},
		{-1, 0, "", "bbp.Reader.ReadAt: negative offset"},
	}
	for i, tt := range tests {
		b := make([]byte, tt.n)
...
func TestReaderMultiBufs(t *testing.T) {
	var buf []byte
	r := NewReader([]byte("0123456789"), []byte("abcdefghij"), []byte("ABCDEFGHIJ"))

	buf = make([]byte, 5)
	n, err := r.Read(buf)
	if string(buf) != "01234" || n != 5 || err != nil {
		t.Errorf("want '01234', n=5 and no error")
	}

	buf = make([]byte, 6)
	n, err = r.Read(buf)
	if string(buf) != "56789a" || n != 6 || err != nil {
		t.Errorf("want '56789a', n=6 and no error")
	}

	// test reading to buf boundary
	r.Seek(5, io.SeekStart)
	buf = make([]byte, 5)
	_, _ = r.Read(buf)
	buf = make([]byte, 5)
	n, err = r.Read(buf)
	if string(buf) != "abcde" || n != 5 || err != nil {
		t.Errorf("want 'abcde', n=5 and no error")
	}

	// test seeking to buf boundary
