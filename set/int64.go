...
func (s *Int64) Size() int { return len(s.m) }
...
func (s Int64) DiffSlice(other []int64) Int64 {
	if len(s.m) > len(other) {
		...
		res := NewInt64WithSize(max(s.Size()-dup, 0))
		for val := range s.m {
...
// the provided slice and also present in the Int64 set.
func (s Int64) FilterInclude(slice []int64) []int64 {
	res := make([]int64, 0, min(s.Size(), len(slice)))
...
// the provided slice but don't present in the Int64 set.
...
// the set will be marshaled as an int64 array.
func (s Int64) MarshalYAML() (interface{}, error) {
	res := s.Slice()
	return res, nil
}

// UnmarshalYAML implements yaml.Unmarshaler interface of the yaml package,
// it will unmarshal an int64 array to the set.
func (s *Int64) UnmarshalYAML(unmarshal func(interface{}) error) error {
	vals := make([]int64, 0)
	err := unmarshal(&vals)
	if err == nil {
		s.Add(vals...)
