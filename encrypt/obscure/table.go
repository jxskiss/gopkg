...
var (
	...
	idxcharsMu      sync.Mutex
	idxcharsChanged bool
...
// SetIdxChars sets character sequence to index encode/decode table,
// this function can be called at most once in a program, and the
// provided character table must be a combination of the 52 uppercase
// and lowercase letters, or it panics.
func SetIdxChars(chars string) {
	idxcharsMu.Lock()
	defer idxcharsMu.Unlock()
	if idxcharsChanged {
		panic("obscure: index chars set more than once")
	}
...
// SetIDCharTable sets character table to encode/decode number ID, this
// function can be called at most once in a program, or it panics.
// The parameter table can be generated by script main.go in this package.
func SetIDCharTable(table Table) {
	idseqcharsMu.Lock()
	defer idseqcharsMu.Unlock()
	if idseqcharsChanged {
		panic("obscure: id chars table set more than once")
	}

	idseqcharsChanged = true
	idseqchars = table
	idseqdec = [seqlen][128]uint8{}
	for i := range idseqdec {
		for j, c := range idseqchars[i] {
...
var (
	...
	b64seqcharsMu      sync.Mutex
	b64seqcharsChanged bool
	b64seqchars        = Table{
...
// SetBase64Table sets character table to encode/decode base64, this
// function can be called at most once in a program, or it panics.
// The parameter table can be generated by script main.go in this package.
func SetBase64Table(table Table) {
	b64seqcharsMu.Lock()
	defer b64seqcharsMu.Unlock()
	if b64seqcharsChanged {
		panic("obscure: base64 table set more than once")
	}

	b64seqcharsChanged = true
	b64seqchars = table
	for i := range b64encodings {
		b64encodings[i] = base64.NewEncoding(b64seqchars[i]).WithPadding(base64.NoPadding)
	}
...
var (
	...
	b62seqcharsMu      sync.Mutex
	b62seqcharsChanged bool
	b62seqchars        = Table{
...
// SetBase62Table sets character table to encode/decode base62, this
// function can be called at most once in a program, or it panics.
// The parameter table can be generated by script main.go in this package.
func SetBase62Table(table Table) {
	b62seqcharsMu.Lock()
	defer b62seqcharsMu.Unlock()
	if b62seqcharsChanged {
		panic("obscure: base62 table set more than once")
	}

	b62seqcharsChanged = true
	b62seqchars = table
	for i := range b62encodings {
		b62encodings[i] = base62.NewEncoding(b62seqchars[i])
	}
